        ...
        <div className="glass-strong rounded-2xl border border-white/10 shadow-2xl flex flex-col h-full animate-slide-up">
          ...
          <div className="flex items-center justify-between p-3 sm:p-4 border-b border-white/10">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full gradient-primary flex items-center justify-center">
                <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
              </div>
              <div>
                <h3 className="font-bold text-white text-sm sm:text-base">Repoza Assistant</h3>
                ...
                >
                  {message.role === 'assistant' ? (
                    <div className="prose prose-invert prose-sm max-w-none">
                      <ReactMarkdown
                        components={{
                          p: ({ node, ...props }) => <p className="mb-2 last:mb-0 text-xs sm:text-sm" {...props} />,
                          strong: ({ node, ...props }) => (
          ...
          <div className="p-3 sm:p-4 border-t border-white/10">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 sm:px-4 sm:py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500 text-xs sm:text-sm"
                disabled={loading}
              />
              <button
                onClick={() => sendMessage()}
                disabled={loading || !input.trim()}
                className="p-2 sm:p-2.5 gradient-primary rounded-lg hover:shadow-glow-cyan transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
